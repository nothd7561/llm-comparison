...
def get_max_tokens(df):
    return df['top_provider']['max_tokens']
def get_moderated(df):
    return df['top_provider']['moderated']
...
data_df['max_tokens'] = data_df.apply(get_max_tokens, axis=1)
data_df['moderated'] = data_df.apply(get_moderated, axis=1)
